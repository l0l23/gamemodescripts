...
public ChargerKilled_Event(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	CPrintToChatAll("%s{green}%N {olive}Level a Charge{default}'d on {green}%N{default} with %s!!", TAG_LEVEL, survivor, charger, weapon);
	CPrintToChat(survivor, "%sYou just {olive}Level a Charge{default}'d on {green}%N{default} with %s!!", TAG_LEVEL, charger, weapon);
	CPrintToChat(charger, "%sYou just got {olive}Level a Charge{default}'d by {green}%N{default} with %s!!", TAG_LEVEL, survivor, weapon);
}
